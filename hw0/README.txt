...
To consider the largest accuracy, we can choose K=499.
However, if we want to pick a reasonable situation which means if you pic "K=499" you have only one predictor based on the data file, which does not make any sense, We need to consider 1)number of predictors is not too small 2)K is not too large which means you do not have to pick so many days for prediction.
