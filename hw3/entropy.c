...
double Entropy(int num_pos, int num_neg) {
  ...
  if (num_pos == 0 || num_neg == 0)
    return entropy;
  double pro_pos, pro_neg;
  pro_pos=((double)num_pos)/(num_pos+num_neg);
  pro_neg=((double)num_neg)/(num_pos+num_neg);
  
  entropy=-pro_pos*LogBase2(pro_pos)-pro_neg*LogBase2(pro_neg);  
