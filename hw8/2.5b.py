...
def train_NB(train_set,M):
    ...
    tot_list=con_list+lib_list
    
    num_tot=len(tot_list)
    word_freq={}
    ...
    for k,v in con_word_freq.items():
 	q=(float(word_freq.get(k))+float(1.0))/(num_tot+num_voc)
	#print q
	score=-math.log((float(v)+float(M)*q)/(con_list_size+M))
	con_dic[k]=score

    for word in vocabulary:
	if con_dic.get(word,"empty")=="empty":
	    q=(float(word_freq.get(word))+float(1.0))/(num_tot+num_voc)
	    con_dic[word]=-math.log((float(M)*q)/(con_list_size+M))
    ...
    for k,v in lib_word_freq.items():
  	q=(float(word_freq.get(k))+float(1.0))/(num_tot+num_voc)
	score=-math.log((float(v)+float(M)*q)/(lib_list_size+M))
	lib_dic[k]=score

    for word in vocabulary:
	if lib_dic.get(word,"empty")=="empty":
	    q=(float(word_freq.get(word))+float(1.0))/(num_tot+num_voc)
	    lib_dic[word]=-math.log((float(M)*q)/(lib_list_size+M))
...
def main():
	...
	train_dic=train_NB(train_set, sys.argv[3])
