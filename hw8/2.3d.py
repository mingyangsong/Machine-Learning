...
def train_NB(train_set):
	...
	score=(float)(v+1)/(con_list_size+num_voc)
	con_dic[k]=score

    for word in vocabulary:
	if con_dic.get(word,"empty")=="empty":
	    con_dic[word]=(float)(1.0)/(con_list_size+num_voc)

    for word in lib_list:
        lib_word_freq[word] = lib_word_freq.get(word,0) + 1  
  
    for k,v in lib_word_freq.items():
	score=(float)(v+1)/(lib_list_size+num_voc)
	lib_dic[k]=score

    for word in vocabulary:
	if lib_dic.get(word,"empty")=="empty":
	    lib_dic[word]=(float)(1.0)/(lib_list_size+num_voc)
    
    log_cl={}
    log_lc={}

    for word in vocabulary:
	log_cl[word]=math.log(con_dic.get(word)/lib_dic.get(word))
    for word in vocabulary:
	log_lc[word]=math.log(lib_dic.get(word)/con_dic.get(word))

    i=0
    for w in sorted(log_cl, key=log_cl.get, reverse=True):
	i=i+1  	
	print w, log_cl[w]
        if i==20:
	    break
    ...
    print "---------------"
    i=0
    for w in sorted(log_lc, key=log_lc.get, reverse=True):
	i=i+1  	
	print w, log_lc[w]
	if i==20:
	    break
