...
def train_NB(train_set,M):
	...
	score=-math.log((float(v)+float(M)/num_voc)/(con_list_size+M))
	con_dic[k]=score

    for word in vocabulary:
	if con_dic.get(word,"empty")=="empty":
	    con_dic[word]=-math.log((float(M)/num_voc)/(con_list_size+M))

    for word in lib_list:
        lib_word_freq[word] = lib_word_freq.get(word,0) + 1  
  
    for k,v in lib_word_freq.items():
	score=-math.log((float(v)+float(M)/num_voc)/(lib_list_size+M))
	lib_dic[k]=score

    for word in vocabulary:
	if lib_dic.get(word,"empty")=="empty":
	    lib_dic[word]=-math.log((float(M)/num_voc)/(lib_list_size+M))
